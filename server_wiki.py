...
def extract_title(text: str, slug: str) -> str:
    ...
    return m.group(1).strip() if m else slug_to_display(slug)
...
def get_categories(topics_dir: Path) -> list:
    ...
    for cat_dir in sorted(topics_dir.iterdir()):
        ...
        for md_file in sorted(cat_dir.iterdir()):
            if md_file.suffix != ".md" or md_file.name.startswith("."):
                continue
            slug = md_file.stem
            text = md_file.read_text(encoding="utf-8")
            topics.append({"slug": slug, "title": extract_title(text, slug)})
        cats.append({
            "slug": cat_dir.name,
    ...
    def do_GET(self):
        ...
        if path == "/api/topic":
            ...
            if not md_path or not md_path.exists():
                ...
                return
            self.send_json({"content": md_path.read_text(encoding="utf-8")})
            return
